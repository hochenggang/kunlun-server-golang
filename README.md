...
### **轻量高效**
- 后端基于 Go + Gin + SQLite，单二进制文件，资源占用极低
- 客户端基于 C 语言实现，性能优异
...
1. **Kunlun Server**（后端）：
   - 基于 Gin 提供 RESTful API，用于接收和存储客户端上报的数据
...
```bash
bash <(curl -sL https://github.com/hochenggang/kunlun-server-golang/raw/main/kunlun-server-golang.sh) install
```
...
```bash
kunlun-server-golang.sh version
```

### 升级到最新版本

```bash
kunlun-server-golang.sh upgrade
```
...
```bash
# 服务管理
kunlun-server-golang.sh status      # 查看服务状态
kunlun-server-golang.sh start       # 启动服务
kunlun-server-golang.sh stop        # 停止服务
kunlun-server-golang.sh restart     # 重启服务
kunlun-server-golang.sh logs        # 查看日志

# 版本管理
kunlun-server-golang.sh version     # 查看版本
kunlun-server-golang.sh upgrade     # 升级版本

# 客户端管理
kunlun-server-golang.sh client list           # 查看所有客户端
kunlun-server-golang.sh client pending        # 查看待审核客户端
kunlun-server-golang.sh client approve <id>   # 审核通过
kunlun-server-golang.sh client reject <id>    # 拒绝客户端
kunlun-server-golang.sh client delete <id>    # 删除客户端
```
...
2. GitHub Actions 自动构建 Linux AMD64 二进制并发布 Release，包含：
   - `kunlun-server` 二进制文件
   - 发布说明
