...
APP_NAME="kunlun-server-golang"
SCRIPT_NAME="kunlun-server-golang.sh"
APP_DIR="/opt/apps/$APP_NAME"
SERVICE_FILE="/etc/systemd/system/$APP_NAME.service"
REPO_URL="https://github.com/hochenggang/kunlun-server-golang"
RELEASE_API="https://api.github.com/repos/hochenggang/kunlun-server-golang/releases/latest"
...
check_app_dir() {
    if ! detect_app_dir; then
        ...
        echo ""
        echo "示例："
        echo "  cd /opt/apps/kunlun-server-golang"
        echo "  ./kunlun-server-golang.sh status"
        exit 1
    fi
}
...
deploy_binary() {
    ...
    local download_url="$REPO_URL/releases/download/$INSTALL_VERSION/kunlun-server"
    
    print_info "下载地址: $download_url"
    
    curl -sL "$download_url" -o "$APP_DIR/kunlun-server"
    chmod +x "$APP_DIR/kunlun-server"
...
ExecStart=$APP_DIR/kunlun-server
Restart=always
RestartSec=5
...
do_upgrade() {
    ...
    print_info "下载新版本..."
    local download_url="$REPO_URL/releases/download/$latest_version/kunlun-server"
    
    curl -sL "$download_url" -o "$APP_DIR/kunlun-server.new"
    chmod +x "$APP_DIR/kunlun-server.new"
    mv "$APP_DIR/kunlun-server.new" "$APP_DIR/kunlun-server"
